...
COpRadialGradient::COpRadialGradient()
{
	// One input
...
TChannel COpRadialGradient::modifyLayer (CFloatBitmap &output, const CRenderParameter &renderParameters)
{
	...
	if (const_cast<const COpRadialGradient*>(this)->getBindedOp(OffsetInput))
	{
	...
	if (dst)
	{
		...
		if (_Parameters[Mirror].Integer)
		{
			...
			for (y=0; y<height; y++, fy += dY)
			{
				...
				{
					float tmp = scaleFactor*(float)sqrt(fx*fx + fy*fy) - offset;
					if (offsetPixels)
					{
						tmp -= *offsetPixels;
						offsetPixels += 4;
